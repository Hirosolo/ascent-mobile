...
export function WorkoutScreen() {
  ...
  const [today, setDay] = useState<Date>(new Date());
  ...
  const muscleSplit = useMemo(() => {
    return [...(summaryQuery.data?.muscle_split ?? [])].sort(
      (a, b) => b.value - a.value,
    );
  ...
  const focusLabel =
    focusItems.length > 1
      ? `${focusItems[0].name}/${focusItems[1].name}`
      : focusItems[0]?.name ?? "No data";
  ...
  const patchPlannedExercise = (
    ...
    setPlannedExercises((prev) =>
      prev.map((item) =>
        item.exercise_id === exerciseId ? { ...item, ...patch } : item,
      ),
    );
  };
  ...
  const exerciseOptions = useMemo<ExerciseOption[]>(() => {
    const rawList = Array.isArray(exercisesQuery.data)
      ? exercisesQuery.data
      : [];
    ...
    return rawList
      .map((item, index) => {
        ...
        const resolvedId =
          Number(source.exercise_id ?? source.id ?? 0) || index + 1;
        const resolvedName =
          source.name ?? source.title ?? `Exercise ${resolvedId}`;
        return {
  ...
  const selectedPlan = useMemo(() => {
    ...
    return (
      (plansQuery.data || []).find((plan) => plan.plan_id === selectedPlanId) ??
      null
    );
  ...
  return (
    ...
    >
      <View style={styles.header}>
        <View>
          <Text style={styles.kicker}>Ascent</Text>
          <Text style={styles.title}>WORKOUT</Text>
          <Text style={styles.dateText}>{format(today, "MMM dd, yyyy")}</Text>
        </View>
        <Pressable
          style={styles.profileBtn}
          onPress={() => navigation.navigate("PlanDayManager")}
        >
      ...
      <View style={styles.metricsRow}>
        <View style={styles.metricCard}>
          ...
          <Text style={styles.metricValue}>{grScore}</Text>
          <View style={styles.metricTrendRow}>
            <MaterialCommunityIcons
              color={grChange >= 0 ? "#34d399" : "#f87171"}
              name={grChange >= 0 ? "trending-up" : "trending-down"}
              size={14}
            />
            <Text
              style={[
                styles.metricTrendText,
                grChange < 0 && styles.metricTrendTextDown,
              ]}
            >
              {grChange >= 0 ? "+" : ""}
              {grChange} GR vs last month
            </Text>
          </View>
        </View>
        ...
        <View style={styles.metricCard}>
          ...
          <View style={styles.metricTrendRow}>
            ...
            <Text style={styles.metricTrendText}>
              Best streak {summaryQuery.data?.longest_streak ?? 0} days
            </Text>
          </View>
        </View>
      </View>
      ...
      <View style={styles.panel}>
        ...
        {focusItems.length === 0 ? (
          <Text style={styles.muted}>
            No completed sessions for this month yet.
          </Text>
        ) : (
          <View style={styles.focusRow}>
            <View style={styles.focusChartWrap}>
              ...
              <View style={styles.focusCenterLabel}>
                <Text style={styles.focusPercent}>
                  {Math.round(focusPrimary?.value ?? 0)}%
                </Text>
              ...
              {focusItems.map((item, idx) => (
                <View key={item.name} style={styles.focusLegendRow}>
                  ...
                  <View style={styles.focusLegendTextWrap}>
                    ...
                    <Text style={styles.focusLegendSub}>
                      {Math.round(item.value)}% split
                    </Text>
                  </View>
                </View>
              ))}
      ...
      <View style={styles.panel}>
        ...
        <View style={styles.weekRow}>
          {["S", "M", "T", "W", "T", "F", "S"].map((d, index) => (
            <Text key={`${d}-${index}`} style={styles.weekCell}>
              {d}
            </Text>
          ))}
        </View>
            ...
            return (
              ...
              >
                <Text
                  style={[
                    styles.dayText,
                    !cell.currentMonth && styles.dayOut,
      ...
      <View style={styles.panel}>
        ...
        {protocolSessions.length === 0 ? (
          <Text style={styles.muted}>
            No workout sessions on the selected day.
          </Text>
        ) : null}
            ...
            >
              ...
              <View style={styles.sessionRight}>
                <Text
                  style={[
                    styles.badge,
                    item.status === "COMPLETED" ? styles.done : styles.progress,
                  ]}
                >
                  {item.status}
                </Text>
                    ...
                    {SESSION_TYPE_OPTIONS.map((option) => (
                      ...
                      >
                        <Text
                          style={[
                            styles.typeChipText,
                            createType === option && styles.typeChipTextActive,
                          ]}
                        >
                          {option}
                        </Text>
                  ...
                  >
                    <View style={styles.selectFieldContent}>
                      <Text style={styles.selectFieldValue}>
                        {selectedPlan?.name || "None (Manual)"}
                      </Text>
                      <MaterialCommunityIcons
                        color={colors.textDim}
                        name={
                          isPlanSelectorOpen ? "chevron-up" : "chevron-down"
                        }
                        size={20}
                      />
                    </View>
              ...
              >
                  ...
                  >
                    ...
                    {groupedExerciseOptions.map((group) => (
                      <View key={group.category}>
                        <Text style={styles.groupCategoryLabel}>
                          {group.category}
                        </Text>
                        {group.items.map((exercise) => {
                          const alreadyAdded = plannedExercises.some(
                            (p) => p.exercise_id === exercise.exercise_id,
                          );
                          return (
                            <Pressable
                            ...
                            >
                              ...
                              <View
                                style={[
                                  styles.addBadge,
                                  alreadyAdded && styles.addBadgeActive,
                                ]}
                              >
                                <Text
                                  style={[
                                    styles.addBadgeText,
                                    alreadyAdded && styles.addBadgeTextActive,
                                  ]}
                                >
                                  {alreadyAdded ? "✓" : "+"}
                                </Text>
                ...
                {plannedExercises.length > 0 ? (
                  <View style={styles.formCard}>
                    ...
                    <ScrollView
                      style={styles.selectedExercisesList}
                      nestedScrollEnabled
                    >
                      {plannedExercises.map((item, idx) => (
                        ...
                        >
                          <View style={styles.selectedExerciseHead}>
                            <Text style={styles.selectedExerciseIndex}>
                              {idx + 1}
                            </Text>
                            ...
                            <Pressable
                              ...
                              onPress={() =>
                                removePlannedExercise(item.exercise_id)
                              }
                            >
                              <MaterialCommunityIcons
                                color={colors.textDim}
                          ...
                          <View style={styles.selectedExerciseInputs}>
                            <View style={styles.inputGroupSmall}>
                              ...
                              <TextInput
                                ...
                                onChangeText={(v) =>
                                  patchPlannedExercise(item.exercise_id, {
                                    planned_sets: Math.max(
                                      1,
                                      Number(v.replace(/[^0-9]/g, "")) || 1,
                                    ),
                                  })
                                }
                              />
            ...
            <View style={styles.modalFooter}>
              ...
              <Pressable
                style={[
                  styles.createBtn,
                  !(
                    (createStep === 1 && createType) ||
                    (createStep === 2 && plannedExercises.length > 0)
